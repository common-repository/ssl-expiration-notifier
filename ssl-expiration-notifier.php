...
class SSL_Expiration_Notifier {
    ...
    public function check_ssl_cert_function() {
        ...
        $current_time = time();
        $expiration_date_text = date('Y-m-d H:i:s', $expiration_date);
        if ($expiration_date - $current_time <= $warning_days * 24 * 60 * 60) {
            ...
            $mailMessage = esc_html__(
                sprintf(
                    "SSL certificate on your website will expire at %s. Please renew it as soon as possible.",
                    $expiration_date_text
                ),
                'ssl-expiration-notifier'
            );
            $logMessageMailSuccessful = esc_html__(
                sprintf(
                    "SSL certificate expiration warning email sent. SSL certificate on your website will will expire at %s.",
                    $expiration_date_text
                ),
                'ssl-expiration-notifier'
            );

            wp_mail(get_option('admin_email'), $subject, $mailMessage);

            $log_content = '[' . esc_html(date('Y-m-d H:i:s')) . '] '.$logMessageMailSuccessful.'' . PHP_EOL;
            file_put_contents($log_file, $log_content, FILE_APPEND);
        } else {
            $logMessage = esc_html__(
                sprintf(
                    "SSL certificate on your website will will expire at %s.",
                    $expiration_date_text
                ),
                'ssl-expiration-notifier'
            );
            $log_content = '[' . esc_html(date('Y-m-d H:i:s')) . '] '.$logMessage.'' . PHP_EOL;
            file_put_contents($log_file, $log_content, FILE_APPEND);
        }
        fclose($stream);
